...
def plot_actual_vs_pred(y_true, y_pred, title, save_path):
    ...
    plt.xlabel("Actual Impp (A)")
    plt.ylabel("Predicted Impp (A)")
    plt.title(title)
    plt.tight_layout()
...
def main():
    ...
    plot_actual_vs_pred(
        y_test_orig,
        y_test_pred,
        "Linear Regression: Actual vs Predicted Impp (Test Set)",
        "06_graphs/impp_linear_regression_actual_vs_pred.png"
    )

    append_metrics(
        filepath="05_ann/metrics/impp_linear_regression_metrics.txt",
        metrics_dict={
        ...
        notes="Linear regression baseline for impp(same split & scaling as ANN)"
    )
    ...
    plot_actual_vs_pred(
        y_test_orig,
        y_test_pred,
        "Ridge Regression: Actual vs Predicted Impp (Test Set)",
        "06_graphs/impp_ridge_regression_actual_vs_pred.png"
    )

    append_metrics(
        filepath="05_ann/metrics/impp_ridge_regression_metrics.txt",
        metrics_dict={
