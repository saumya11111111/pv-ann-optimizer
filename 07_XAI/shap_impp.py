...
from utils import load_data, prepare_X_y_impp

# ---- Create output directory ----
os.makedirs("06_graphs/explainable_graphs", exist_ok=True)

# ---- Load trained Impp model ----
model = load_model("05_training/models/impp_model.h5", compile=False)

# ---- Load data ----
df = load_data("03_data/PV_results.csv")
X, y = prepare_X_y_impp(df)   # X = [G, Temp]
...
plt.tight_layout()
plt.savefig(
    "06_graphs/explainable_graphs/shap_impp_summary_full_test.png",
    dpi=300
)
plt.close()

print("Saved SHAP Impp summary plot (full test set).")
